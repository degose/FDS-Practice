  ...
  var Notification = function () {
    ...
    function Notification(selector) {
      ...
      if ($.type(selector) !== 'string') {
        throw 'CSS 선택자(문자열)를 전달해야 합니다.';
      }
      // 빈 문자열일 경우 -> 오류 메세지
      // if ( selector.length < 1 ) { throw '빈 문자열 입니다. 아무말이라도 던져주세요.' }
      if (selector.trim() === '') {
        throw '빈 문자열 입니다. 아무말이라도 던져주세요.';
      }
      // new 를 붙이지 않고 값을 전달하는 경우 -> 오류 메세지 => ???
      if (!this) {
        throw 'new Notificatoin()으로 실행해주세요.';
      }
      ...
      this.$selectors.each(function (i) {
        ...
        var $selector_index = _this.$selectors.eq(i);
        // 그 중에 .delete를 가진것을 찾아서 click 이벤트를 주고, close 함수를 바인딩
        $selector_index.find('.delete').on('click', _this.close.bind($selector_index));
      });
    }
